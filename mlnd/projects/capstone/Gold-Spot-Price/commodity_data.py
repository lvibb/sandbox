...
def train_test_split_linear_regression(commodity):
    ...
    split = .315
    split = int(math.floor(len(commodity['Item']) * split))
...
def train_test_split_lstm(commodity, prediction_time=1, test_data_size=1650, unroll_length=50):
    ...
    x_train = commodity[0:-prediction_time - test_data_cut].as_matrix()
    y_train = commodity[prediction_time:-test_data_cut]['Close'].as_matrix()

    # test data
    x_test = commodity[0 - test_data_cut:-prediction_time].as_matrix()
    y_test = commodity[prediction_time - test_data_cut:]['Close'].as_matrix()
